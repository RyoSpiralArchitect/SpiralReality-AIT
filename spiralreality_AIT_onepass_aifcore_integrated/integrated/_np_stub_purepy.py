    ...
    def astype(self, dtype) -> "ndarray":
        def cast(v):
            ...
            if dtype in (bool, "bool"):
                return bool(v)
            return dtype(v)
        result = ndarray(_elementwise(lambda x, _: cast(x), self._data, 0.0))
        result._dtype = float if dtype in (float, "float", "float32", "float64") else dtype
        return result
...
def mean(arr, axis=None, keepdims=False):
    ...
    if axis is None:
        ...
        if not values:
            return 0.0
        return float(builtins.sum(values) / len(values))
    if axis == 0:
        cols = list(zip(*arr._data)) if arr._data else []
        vals = [float(builtins.sum(col) / len(col)) for col in cols]
        if keepdims:
            return ndarray([vals])
        return ndarray(vals)
    if axis == 1:
        vals = [float(builtins.sum(row) / len(row)) if row else 0.0 for row in arr._data]
        if keepdims:
            return ndarray([[v] for v in vals])
        return ndarray(vals)
    raise ValueError("Unsupported axis")
...
def _variance(values: List[float], ddof: int) -> float:
    n = len(values)
    if n == 0 or ddof >= n:
        return 0.0
    mean_val = builtins.sum(values) / n
    return builtins.sum((x - mean_val) ** 2 for x in values) / (n - ddof)


def std(arr, axis=None, ddof: int = 0, keepdims: bool = False):
    arr = _ensure_ndarray(arr)
    if axis is None:
        values = _flatten(arr._data)
    ...
    if axis == 0:
        cols = list(zip(*arr._data)) if arr._data else []
        vals = [math.sqrt(_variance(list(col), ddof)) for col in cols]
        if keepdims:
            return ndarray([vals])
        return ndarray(vals)
    if axis == 1:
        vals = [math.sqrt(_variance(list(row), ddof)) for row in arr._data]
        if keepdims:
            return ndarray([[v] for v in vals])
        return ndarray(vals)
    raise ValueError("Unsupported axis")
...
def var(arr, axis=None, ddof: int = 0, keepdims: bool = False):
    arr = _ensure_ndarray(arr)
    if axis is None:
        return float(_variance(_flatten(arr._data), ddof))
    if axis == 0:
        cols = list(zip(*arr._data)) if arr._data else []
        vals = [float(_variance(list(col), ddof)) for col in cols]
        if keepdims:
            return ndarray([vals])
        return ndarray(vals)
    if axis == 1:
        vals = [float(_variance(list(row), ddof)) for row in arr._data]
        if keepdims:
            return ndarray([[v] for v in vals])
        return ndarray(vals)
    raise ValueError("Unsupported axis")
...
def maximum(a, b):
    ...
    return ndarray(_elementwise(builtins.max, a_data, b_data))
...
def minimum(a, b):
    ...
    a_data = _ensure_ndarray(a)._data if not isinstance(a, (int, float)) else a
    b_data = _ensure_ndarray(b)._data if not isinstance(b, (int, float)) else b
    return ndarray(_elementwise(builtins.min, a_data, b_data))


def min(arr, axis=None, keepdims=False):
    arr = _ensure_ndarray(arr)
    if axis is None:
        return builtins.min(_flatten(arr._data))
    if arr.ndim != 2:
        raise ValueError("axis operations only supported for 2D arrays")
    if axis == -1:
        axis = 1
    if axis == 0:
        cols = zip(*arr._data) if arr._data else []
        vals = [builtins.min(col) for col in cols]
        if keepdims:
            return ndarray([vals])
        return ndarray(vals)
    if axis == 1:
        vals = [builtins.min(row) for row in arr._data]
        if keepdims:
            return ndarray([[v] for v in vals])
        return ndarray(vals)
    raise ValueError("Unsupported axis")
...
def matmul(a, b):
    return _ensure_ndarray(a).__matmul__(b)


def exp(x):
    def _safe_exp(v):
        if v > SAFE_EXP_CLIP:
            return math.exp(SAFE_EXP_CLIP)
        if v < -SAFE_EXP_CLIP:
            return math.exp(-SAFE_EXP_CLIP)
        return math.exp(v)
    if isinstance(x, ndarray):
...
def safe_exp(x):
    if isinstance(x, ndarray):
        return exp(x)
    clipped = max(-SAFE_EXP_CLIP, min(SAFE_EXP_CLIP, float(x)))
