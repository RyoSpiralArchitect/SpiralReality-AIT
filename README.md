...
- Hybrid boundary learner: learnable char embeddings → shallow tanh block → binary CRF with
  ...
  `integrated/boundary_cpp.py` and `integrated/boundary_julia.py` detect bespoke C++/Julia/R
  implementations and seamlessly delegate training/decoding when available, otherwise falling
  back to the pure NumPy student.
- Streaming-friendly trainer: `StudentTrainingConfig(cache_sequences=False)` turns on
  ...
  report cache usage so large-scale jobs remain observable.
